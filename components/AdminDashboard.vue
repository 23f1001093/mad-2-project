<template>
  <div class="container mt-5">
    ...
    <div class="d-flex justify-content-between align-items-center mt-4 mb-3">
      <h4>All Quizzes</h4>
      <router-link to="/create-quiz" class="btn btn-success">
        + Create New Quiz
      </router-link>
    </div>

    <div v-if="quizzes.length === 0" class="alert alert-warning">
      No quizzes available.
    </div>

    <ul class="list-group" v-else>
      <li class="list-group-item d-flex justify-content-between align-items-center" v-for="quiz in quizzes" :key="quiz.id">
        {{ quiz.name }} - ({{ getSubjectName(getChapterSubjectId(quiz.chapter_id)) }} / {{ getChapterName(quiz.chapter_id) }})
        <div>
...
export default {
  ...
  data() {
    return {
      ...
      messageClass: '', // For styling the message alert
    };
  },
  methods: {
    async fetchQuizzes() {
      ...
      this.messageClass = '';
      try {
        // This URL is already correct: /api/admin/quizzes
